...
module.exports = function (config) {
    ...
    try {
        ...
        console.log('没有swagger文件或没有权限访问此文件或文件不是合法json格式'.red);
        throw err;
    }
    ...
    _.forEach(swagger.paths, function (value, key) {
        _.forEach(value, function (item, index) {
            ...
            // 按tags分组
            if (tagsArr) {
                _.forEach(item.tags, function (tag, tIndex) {
                    swagger.pathsWithTags[tag] = swagger.pathsWithTags[tag] || {};
                    swagger.pathsWithTags[tag][key] = swagger.pathsWithTags[tag][key] || {};
                    swagger.pathsWithTags[tag][key][index] = _.cloneDeep(item);
                });
            }
        });
    });
