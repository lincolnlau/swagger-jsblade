...
function outputFile(outPath, apiName, str) {
    var filePath = path.join(outPath, apiName + '.js');
...
function outPutApiFile(_configObj, outPath, _apiName, type, surroundMode, customTplPath, filePath, tagName) {
...
module.exports = function (config) {
    ...
    if (!tagsArr) {
        outPutApiFile(configObj, outPath, apiName, type, surroundMode, customTplPath, filePath);
    } else {
        if (tagsArr.length === 0) {
        ...
        tagsArr.forEach(function (item, index) {
            outPutApiFile(configObj, outPath, apiName, type, surroundMode, customTplPath, filePath, item);
        });
    }
};